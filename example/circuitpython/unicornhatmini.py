...
BUTTON_A = board_bcm.BCM5
BUTTON_B = board_bcm.BCM6
BUTTON_X = board_bcm.BCM16
BUTTON_Y = board_bcm.BCM24
...
class UnicornHATMini():
    ...
    def __init__(self, spi_max_speed_hz=600000):
        ...
        self.spi = busio.SPI(board_bcm.BCM11, MOSI=board_bcm.BCM10)
        while not self.spi.try_lock():
            pass
        self.spi.configure(baudrate=spi_max_speed_hz)
        self.spi.unlock()
        ce0 = DigitalInOut(board_bcm.BCM8)
        ce0.direction = Direction.OUTPUT
        ce1 = DigitalInOut(board_bcm.BCM7)
        ce1.direction = Direction.OUTPUT
        self.left_matrix = (self.spi, ce0, 0)
        self.right_matrix = (self.spi, ce1, 28 * 8)
    ...
    def xfer(self, device, pin, command):
        while not device.try_lock():
            pass
        pin.value = 0
        res = bytearray(len(command))
        device.write_readinto(bytearray(command), res)
        pin.value = 1
        device.unlock()
...
if __name__ == "__main__":
    ...
    for i in range(1, 4):
        unicornhatmini.set_all((1 - 1 % i) * 255, 3 % i *255, 4 % i * 255)
        unicornhatmini.show()
        time.sleep(0.5)
    ...
    while True:
        for i,b in enumerate(buttons):
            btn = b.value
            c = 0 if btn else 255
            unicornhatmini.set_pixel(p[i][0], p[i][1], c, c, c)
        unicornhatmini.show()